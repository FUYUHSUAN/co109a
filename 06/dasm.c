    ...
    for(int PC = 0;PC<imTop; PC++){
        ...
        if((I & 0x8000) ==0 ){ //A指令
            ...
            switch (c) { // 處理 c1..6, 計算 aluOut
                case 0x2A: sprintf(op, "0"); break;      // "0",   "101010" sprintf印到字串裡面
                ...
                case 0x37: sprintf(op, "%c+1");break;    // "AM+1"，"110111"
                ...
                default: assert(0); //用於除錯用，assert(0)==>表示直接false
            }
...
int main(int argc, char *argv[]){
    ...
    FILE *binFile = fopen(binFileName, "rb"); //fopen ==> 開檔案
    imTop = fread(im, sizeof(uint16_t), 32768, binFile); //讀檔
    fclose(binFile); //關檔
    disasm(im, imTop); //返組譯
    return 0;
}